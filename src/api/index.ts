    ...
    public static async updateTodoItem(body: TodoItem): Promise<string> {
        try {
            const response = await net.fetch(
                api_constants.backend_url + `todolist/update/${body.id}`,
                {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(body),
...
export const ipcCalls = {
    fetchUser: (body: { email: string; password: string }) => ipcRenderer.invoke('fetchUser', body),

    register: (body: { name: string; email: string; password: string }) =>
        ipcRenderer.invoke('register', body),

    createTodoItem: (body: TodoItem) => ipcRenderer.invoke('createTodoItem', body),

    fetchTodoItem: (body: { name: string }) => ipcRenderer.invoke('fetchTodoItem', body),

    updateTodoItem: (body: TodoItem) => ipcRenderer.invoke('updateTodoItem', body),
}
