...
<template>
    <div
        class="w-full h-screen flex items-center justify-center jetbrains-mono-code bg-[#141414] text-lg text-white"
    >
        <form
            v-if="authorizationType === 'login'"
            @submit.prevent="submitUser"
            class="w-1/3 h-1/3 p-3 flex justify-center items-center"
        >
            <div class="w-fit flex flex-col gap-y-16">
                ...
                <div class="flex flex-row gap-x-4 items-center self-end">
                    <div
                        class="opacity-50 cursor-pointer hover:opacity-100 transition-all ease-linear duration-100"
                        @click="authorizationType = 'register'"
                    >
                        Register
                    </div>
                    <button
                        type="submit"
                        class="rounded-2xl p-2 bg-[#F4511E] cursor-pointer hover:bg-[#f37852] transition-all ease-linear duration-100"
                    >
                        Login
                    </button>
                </div>
